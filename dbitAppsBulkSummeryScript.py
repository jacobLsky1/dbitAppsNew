...
for row in all_apps_records:
    ...
    if appLogger != "null":
        ...
        print(f"created table {appName + '_summery'}")

        # getting distinct dates from table
        postgres_select_date_Query = sql.SQL("SELECT DISTINCT date FROM {tbl_name};").format(
            tbl_name=sql.Identifier(appListId)
        )
        sqlCursor.execute(postgres_select_date_Query)
        dateList = sqlCursor.fetchall()
        ...
        for date in dateList:
            ...
            for user in inValidUsersList:
                ...
                userIP = user[6]
                userAgent = user[7]
                dataStr = ""+userTime +"|"+ userValid +"|"+ userDevice +"|"+ userCode +"|"+ userArea +"|"+ userIP
